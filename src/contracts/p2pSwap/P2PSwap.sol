...
contract P2PSwap is StakingServiceHooks {
    ...
    address constant MATE_TOKEN_ADDRESS = 0x0000000000000000000000000000000000000001;
     ...
     */
    constructor(address _evvmAddress, address _stakingAddress, address _owner) StakingServiceHooks(_stakingAddress) {
        evvmAddress = _evvmAddress;
        owner = _owner;
        maxLimitFillFixedFee = 0.001 ether;
        percentageFee = 500;
        rewardPercentage = Percentage({seller: 5000, service: 4000, mateStaker: 1000});
    ...
    function makeOrder(
        ...
        makePay(
            user, metadata.tokenA, _nonce_Evvm, metadata.amountA, _priorityFee_Evvm, _priority_Evvm, _signature_Evvm
        );
        ...
        ordersInsideMarket[market][orderId] = Order(user, metadata.amountA, metadata.amountB);
    ...
    function cancelOrder(
        ...
        if (market == 0 || ordersInsideMarket[market][metadata.orderId].seller != user) {
            revert("Invalid order");
        }

        if (_priorityFee_Evvm > 0) {
            makePay(user, MATE_TOKEN_ADDRESS, _nonce_Evvm, 0, _priorityFee_Evvm, _priority_Evvm, _signature_Evvm);
        }

        makeCaPay(user, metadata.tokenA, ordersInsideMarket[market][metadata.orderId].amountA);
        ...
        if (Evvm(evvmAddress).isAddressStaker(msg.sender)) {
            makeCaPay(
                ...
                MATE_TOKEN_ADDRESS,
                _priorityFee_Evvm > 0
                    ? ((Evvm(evvmAddress).getRewardAmount() * 3) + _priorityFee_Evvm)
                    : (Evvm(evvmAddress).getRewardAmount() * 2)
            );
        }
        marketMetadata[market].ordersAvailable--;
    ...
    function dispatchOrder_fillPropotionalFee(
        ...
        if (market == 0 || ordersInsideMarket[market][metadata.orderId].seller == address(0)) {
            revert();
        }

        uint256 fee = calculateFillPropotionalFee(ordersInsideMarket[market][metadata.orderId].amountB);

        if (metadata.amountOfTokenBToFill < ordersInsideMarket[market][metadata.orderId].amountB + fee) {
        ...
        if (metadata.amountOfTokenBToFill > ordersInsideMarket[market][metadata.orderId].amountB + fee) {
            makeCaPay(
                user,
                metadata.tokenB,
                metadata.amountOfTokenBToFill - (ordersInsideMarket[market][metadata.orderId].amountB + fee)
            );
        }

        EvvmStructs.DisperseCaPayMetadata[] memory toData = new EvvmStructs.DisperseCaPayMetadata[](2);

        uint256 sellerAmount =
            ordersInsideMarket[market][metadata.orderId].amountB + ((fee * rewardPercentage.seller) / 10_000);
        uint256 executorAmount = _priorityFee_Evvm + ((fee * rewardPercentage.mateStaker) / 10_000);

        // pay seller
        toData[0] = EvvmStructs.DisperseCaPayMetadata(sellerAmount, ordersInsideMarket[market][metadata.orderId].seller);
        // pay executor
        toData[1] = EvvmStructs.DisperseCaPayMetadata(executorAmount, msg.sender);

        balancesOfContract[metadata.tokenB] += (fee * rewardPercentage.service) / 10_000;

        makeDisperseCaPay(toData, metadata.tokenB, toData[0].amount + toData[1].amount);
    ...
    function dispatchOrder_fillFixedFee(
        ...
        (uint256 fee, uint256 fee10) =
            calculateFillFixedFee(ordersInsideMarket[market][metadata.orderId].amountB, maxFillFixedFee);

        if (metadata.amountOfTokenBToFill < ordersInsideMarket[market][metadata.orderId].amountB + fee - fee10) {
            revert();
        }
        ...
        uint256 finalFee = metadata.amountOfTokenBToFill
            ...
            && metadata.amountOfTokenBToFill < ordersInsideMarket[market][metadata.orderId].amountB + fee
            ? metadata.amountOfTokenBToFill - ordersInsideMarket[market][metadata.orderId].amountB
            : fee;
        ...
        toData[0] = EvvmStructs.DisperseCaPayMetadata(
            ordersInsideMarket[market][metadata.orderId].amountB + ((finalFee * rewardPercentage.seller) / 10_000),
            ordersInsideMarket[market][metadata.orderId].seller
        );
        toData[1] = EvvmStructs.DisperseCaPayMetadata(
            _priorityFee_Evvm + ((finalFee * rewardPercentage.mateStaker) / 10_000), msg.sender
        );

        balancesOfContract[metadata.tokenB] += (finalFee * rewardPercentage.service) / 10_000;
     ...
     */
    function calculateFillPropotionalFee(uint256 amount) internal view returns (uint256 fee) {
     ...
     */
    function calculateFillFixedFee(uint256 amount, uint256 maxFillFixedFee)
        internal
        view
        returns (uint256 fee, uint256 fee10)
    {
     ...
     */
    function createMarket(address tokenA, address tokenB) internal returns (uint256) {
        marketCount++;
     ...
     * @param _ammount_Evvm Amount of tokens to transfer
     */
    function makeCaPay(address _user_Evvm, address _token_Evvm, uint256 _ammount_Evvm) internal {
     ...
     */
    function makeDisperseCaPay(EvvmStructs.DisperseCaPayMetadata[] memory toData, address token, uint256 amount)
        internal
    {
    ...
    function rejectProposeOwner() external {
        if (msg.sender != owner_proposal || block.timestamp > owner_timeToAccept) {
            revert();
        }
    ...
    function rejectProposeFillFixedPercentage() external {
        if (msg.sender != owner || block.timestamp > rewardPercentage_timeToAcceptNewChange) {
            revert();
        }
    ...
    function rejectProposePercentageFee() external {
        if (msg.sender != owner || block.timestamp > percentageFee_timeToAccept) {
            revert();
        }
    ...
    function rejectProposeMaxLimitFillFixedFee() external {
        if (msg.sender != owner || block.timestamp > maxLimitFillFixedFee_timeToAccept) {
            revert();
        }
    ...
    function proposeWithdrawal(address _tokenToWithdraw, uint256 _amountToWithdraw, address _to) external {
        if (msg.sender != owner || _amountToWithdraw > balancesOfContract[_tokenToWithdraw]) {
            revert();
        }
     ...
     */
    function getAllMarketOrders(uint256 market) public view returns (OrderForGetter[] memory orders) {
     ...
     */
    function getOrder(uint256 market, uint256 orderId) public view returns (Order memory order) {
     ...
     */
    function findMarket(address tokenA, address tokenB) public view returns (uint256) {
     ...
     */
    function getMarketMetadata(uint256 market) public view returns (MarketInformation memory) {
     ...
     */
    function getAllMarketsMetadata() public view returns (MarketInformation[] memory) {
        MarketInformation[] memory markets = new MarketInformation[](marketCount + 1);
     ...
     */
    function checkIfANonceP2PSwapIsUsed(address user, uint256 nonce) public view returns (bool) {
