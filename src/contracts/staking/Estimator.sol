...
contract Estimator {
    ...
    constructor(address _activator, address _evvmAddress, address _addressStaking, address _admin) {
    ...
    function notifyNewEpoch(address tokenPool, uint256 totalPool, uint256 totalStaked, uint256 tStart)
        public
        onlyActivator
    {
        epoch = EpochMetadata({
        ...
        )
    {
        ...
        uint256 size = Staking(addressStaking.actual).getSizeOfAddressHistory(_user);

        for (uint256 i = 0; i < size; i++) {
            h = Staking(addressStaking.actual).getAddressHistoryByIndex(_user, i);
        ...
        amountTotalToBeRewarded = (averageSm * (epoch.totalPool / epoch.totalStaked)) / 1e18;
