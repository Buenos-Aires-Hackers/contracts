...
contract Staking {
    ...
    function prepareServiceStaking(uint256 amountOfStaking) external onlyCA {
        serviceStakingData = ServiceStakingMetadata({
            ...
            amountServiceBeforeStaking: Evvm(EVVM_ADDRESS).getBalance(msg.sender, PRINCIPAL_TOKEN_ADDRESS),
            amountStakingBeforeStaking: Evvm(EVVM_ADDRESS).getBalance(address(this), PRINCIPAL_TOKEN_ADDRESS)
        });
    }
    ...
    function confirmServiceStaking() external onlyCA {
        ...
        uint256 actualServiceBalance = Evvm(EVVM_ADDRESS).getBalance(msg.sender, PRINCIPAL_TOKEN_ADDRESS);

        uint256 actualStakingBalance = Evvm(EVVM_ADDRESS).getBalance(address(this), PRINCIPAL_TOKEN_ADDRESS);

        if (
            serviceStakingData.amountServiceBeforeStaking - totalStakingRequired != actualServiceBalance
                && serviceStakingData.amountStakingBeforeStaking + totalStakingRequired != actualStakingBalance
        ) {
            revert ErrorsLib.ServiceDoesNotFulfillCorrectStakingAmount(totalStakingRequired);
        }
    ...
    function serviceUnstaking(uint256 amountOfStaking) external onlyCA {
        stakingBaseProcess(
            AccountMetadata({Address: msg.sender, IsAService: true}), false, amountOfStaking, 0, 0, false, ""
        );
    }
    ...
    function stakingBaseProcess(
        ...
        if (isStaking) {
            if (getTimeToUserUnlockStakingTime(account.Address) > block.timestamp) {
            ...
            auxSMsteBalance = userHistory[account.Address].length == 0
                ? amountOfStaking
                : userHistory[account.Address][userHistory[account.Address].length - 1].totalStaked + amountOfStaking;
        } else {
            if (amountOfStaking == getUserAmountStaked(account.Address)) {
                if (getTimeToUserUnlockFullUnstakingTime(account.Address) > block.timestamp) {
            ...
            makeCaPay(PRINCIPAL_TOKEN_ADDRESS, account.Address, (PRICE_OF_STAKING * amountOfStaking));
        }
        ...
        if (Evvm(EVVM_ADDRESS).isAddressStaker(msg.sender) && !account.IsAService) {
            makeCaPay(
