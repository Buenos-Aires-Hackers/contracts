...
contract NameService {
    ...
    mapping(string username => IdentityBaseMetadata basicMetadata) private identityDetails;
    ...
    mapping(string username => mapping(uint256 id => OfferMetadata)) private usernameOffers;

    /// @dev Nested mapping: username => metadata key => custom value string
    mapping(string username => mapping(uint256 numberKey => string customValue)) private identityCustomMetadata;
    ...
    function preRegistrationUsername(
        ...
        if (
            !SignatureUtils.verifyMessageSignedForPreRegistrationUsername(
                Evvm(evvmAddress.current).getEvvmID(), user, hashPreRegisteredUsername, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();

        if (priorityFee_EVVM > 0) {
            makePay(user, 0, priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM);
        }
        ...
        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(msg.sender, Evvm(evvmAddress.current).getRewardAmount() + priorityFee_EVVM);
        }
    }
    ...
    function registrationUsername(
        ...
        if (
            !SignatureUtils.verifyMessageSignedForRegistrationUsername(
                Evvm(evvmAddress.current).getEvvmID(), user, username, clowNumber, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();

        makePay(user, getPriceOfRegistration(username), priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM);

        string memory _key = string.concat("@", AdvancedStrings.bytes32ToString(hashUsername(username, clowNumber)));

        if (identityDetails[_key].owner != user || identityDetails[_key].expireDate > block.timestamp) {
        ...
        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(msg.sender, (50 * Evvm(evvmAddress.current).getRewardAmount()) + priorityFee_EVVM);
        }
    ...
    function makeOffer(
        ...
        if (
            identityDetails[username].flagNotAUsername == 0x01 || !verifyIfIdentityExists(username) || amount == 0
                || expireDate <= block.timestamp
        ) revert ErrorsLib.PreRegistrationNotValid();

        if (
            !SignatureUtils.verifyMessageSignedForMakeOffer(
                Evvm(evvmAddress.current).getEvvmID(), user, username, expireDate, amount, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();

        makePay(user, amount, priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM);
        ...
        usernameOffers[username][offerID] =
            OfferMetadata({offerer: user, expireDate: expireDate, amount: ((amount * 995) / 1000)});

        makeCaPay(
            msg.sender, Evvm(evvmAddress.current).getRewardAmount() + ((amount * 125) / 100_000) + priorityFee_EVVM
        );
        principalTokenTokenLockedForWithdrawOffers += ((amount * 995) / 1000) + (amount / 800);
    ...
    function withdrawOffer(
        ...
        if (
            !SignatureUtils.verifyMessageSignedForWithdrawOffer(
                Evvm(evvmAddress.current).getEvvmID(), user, username, offerID, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();
        ...
        makeCaPay(
            msg.sender,
            Evvm(evvmAddress.current).getRewardAmount() + ((usernameOffers[username][offerID].amount * 1) / 796)
                + priorityFee_EVVM
        );

        principalTokenTokenLockedForWithdrawOffers -=
            (usernameOffers[username][offerID].amount) + (((usernameOffers[username][offerID].amount * 1) / 199) / 4);
    ...
    function acceptOffer(
        ...
        bool priorityFlag_EVVM,
        bytes memory signature_EVVM
    ) public onlyOwnerOfIdentity(user, username) verifyIfNonceIsAvailable(user, nonce) {
        if (
            usernameOffers[username][offerID].offerer == address(0)
        ...
        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(
                ...
                (Evvm(evvmAddress.current).getRewardAmount())
                    + (((usernameOffers[username][offerID].amount * 1) / 199) / 4) + priorityFee_EVVM
            );
        }
    ...
    function renewUsername(
        ...
        if (
            !SignatureUtils.verifyMessageSignedForRenewUsername(
                Evvm(evvmAddress.current).getEvvmID(), user, username, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();

        uint256 priceOfRenew = seePriceToRenew(username);

        makePay(user, priceOfRenew, priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM);

        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(
                msg.sender, Evvm(evvmAddress.current).getRewardAmount() + ((priceOfRenew * 50) / 100) + priorityFee_EVVM
            );
        }
    ...
    function addCustomMetadata(
        ...
        bool priorityFlag_EVVM,
        bytes memory signature_EVVM
    ) public onlyOwnerOfIdentity(user, identity) verifyIfNonceIsAvailable(user, nonce) {
        if (bytes(value).length == 0) revert ErrorsLib.EmptyCustomMetadata();

        if (
            !SignatureUtils.verifyMessageSignedForAddCustomMetadata(
                Evvm(evvmAddress.current).getEvvmID(), user, identity, value, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();

        makePay(user, getPriceToAddCustomMetadata(), priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM);

        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(
                msg.sender,
                (5 * Evvm(evvmAddress.current).getRewardAmount()) + ((getPriceToAddCustomMetadata() * 50) / 100)
                    + priorityFee_EVVM
            );
        }

        identityCustomMetadata[identity][identityDetails[identity].customMetadataMaxSlots] = value;
    ...
    function removeCustomMetadata(
        ...
        if (
            !SignatureUtils.verifyMessageSignedForRemoveCustomMetadata(
                Evvm(evvmAddress.current).getEvvmID(), user, identity, key, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();
        ...
        makePay(user, getPriceToRemoveCustomMetadata(), priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM);

        if (identityDetails[identity].customMetadataMaxSlots == key) {
            delete identityCustomMetadata[identity][key];
        } else {
            for (uint256 i = key; i < identityDetails[identity].customMetadataMaxSlots; i++) {
                identityCustomMetadata[identity][i] = identityCustomMetadata[identity][i + 1];
            }
        ...
        nameServiceNonce[user][nonce] = true;
        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(msg.sender, (5 * Evvm(evvmAddress.current).getRewardAmount()) + priorityFee_EVVM);
        }
    }
    ...
    function flushCustomMetadata(
        ...
        if (
            !SignatureUtils.verifyMessageSignedForFlushCustomMetadata(
                Evvm(evvmAddress.current).getEvvmID(), user, identity, nonce, signature
            )
        ) revert ErrorsLib.InvalidSignatureOnNameService();
        ...
        for (uint256 i = 0; i < identityDetails[identity].customMetadataMaxSlots; i++) {
        ...
        if (Evvm(evvmAddress.current).isAddressStaker(msg.sender)) {
            makeCaPay(
                msg.sender,
                ((5 * Evvm(evvmAddress.current).getRewardAmount()) * identityDetails[identity].customMetadataMaxSlots)
                    + priorityFee_EVVM
            );
        }
    ...
    function flushUsername(
        ...
        bool priorityFlag_EVVM,
        bytes memory signature_EVVM
    ) public verifyIfNonceIsAvailable(user, nonce) onlyOwnerOfIdentity(user, username) {
        if (
        ...
        makePay(
            user, getPriceToFlushUsername(username), priorityFee_EVVM, nonce_EVVM, priorityFlag_EVVM, signature_EVVM
        );

        for (uint256 i = 0; i < identityDetails[username].customMetadataMaxSlots; i++) {
            delete identityCustomMetadata[username][i];
        }

        makeCaPay(
            msg.sender,
            ((5 * Evvm(evvmAddress.current).getRewardAmount()) * identityDetails[username].customMetadataMaxSlots)
                + priorityFee_EVVM
        );
    ...
    function acceptProposeAdmin() public {
        ...
        admin = AddressTypeProposal({current: admin.proposal, proposal: address(0), timeToAccept: 0});
    }
    ...
    function proposeWithdrawPrincipalTokens(uint256 _amount) public onlyAdmin {
        if (
            Evvm(evvmAddress.current).getBalance(address(this), PRINCIPAL_TOKEN_ADDRESS)
                - (5083 + Evvm(evvmAddress.current).getRewardAmount() + principalTokenTokenLockedForWithdrawOffers)
                < _amount || _amount == 0
        ) {
            revert();
        }
    ...
    function acceptChangeEvvmAddress() public onlyAdmin {
        ...
        evvmAddress = AddressTypeProposal({current: evvmAddress.proposal, proposal: address(0), timeToAccept: 0});
    }
     ...
     */
    function isValidPhoneNumberNumber(string memory _phoneNumber) internal pure returns (bool) {
        bytes memory _telephoneNumberBytes = bytes(_phoneNumber);
        if (_telephoneNumberBytes.length < 20 && _telephoneNumberBytes.length > 5) {
            revert();
        }
    ...
    function isValidEmail(string memory _email) internal pure returns (bool) {
        ...
        for (uint256 point = 0; point < _emailBytes.length; point++) {
            //step 1 0x00 prefix
            if (flagVerify == 0x00) {
    ...
    function _isLetter(bytes1 character) private pure returns (bool) {
        return ((character >= 0x41 && character <= 0x5A) || (character >= 0x61 && character <= 0x7A)); // ASCII ranges for letters A-Z and a-z
    }
    ...
    function _isAnySimbol(bytes1 character) private pure returns (bool) {
        return (
            ...
            /// @dev includes characters from "!" to "/"
    ...
    function _isOnlyEmailPrefixCharacters(bytes1 character) private pure returns (bool) {
        return (
            _isLetter(character) || _isDigit(character) || (character >= 0x21 && character <= 0x2F)
            /// @dev includes characters from "!" to "/"
            || (character >= 0x3A && character <= 0x3F)
            /// @dev includes characters from ":" to "?"
            || (character >= 0x5B && character <= 0x60)
            /// @dev includes characters from "[" to "`"
            || (character >= 0x7B && character <= 0x7E)
     ...
     */
    function hashUsername(string memory _username, uint256 _randomNumber) public pure returns (bytes32) {
     ...
     */
    function verifyIfIdentityExists(string memory _identity) public view returns (bool) {
        if (identityDetails[_identity].flagNotAUsername == 0x01) {
            if (identityDetails[_identity].owner == address(0) || identityDetails[_identity].expireDate != 0) {
                return false;
            } else {
                return true;
            }
     ...
     */
    function strictVerifyIfIdentityExist(string memory _username) public view returns (bool) {
        if (identityDetails[_username].flagNotAUsername == 0x01) {
            if (identityDetails[_username].owner == address(0) || identityDetails[_username].expireDate != 0) {
                revert();
            } else {
                return true;
            }
     ...
     */
    function getOwnerOfIdentity(string memory _username) public view returns (address) {
     ...
     */
    function verifyStrictAndGetOwnerOfIdentity(string memory _username) public view returns (address answer) {
        ...
        if (identityDetails[_identity].expireDate >= block.timestamp) {
            if (usernameOffers[_identity][0].expireDate != 0) {
                for (uint256 i = 0; i < identityDetails[_identity].offerMaxSlots; i++) {
                    if (
                        usernameOffers[_identity][i].expireDate > block.timestamp
            ...
            if (price == 0) {
                ...
                uint256 principalTokenReward = Evvm(evvmAddress.current).getRewardAmount();
     ...
     * @return price Cost in Principal Tokens (10x current reward amount)
     */
    function getPriceToRemoveCustomMetadata() public view returns (uint256 price) {
        price = 10 * Evvm(evvmAddress.current).getRewardAmount();
    }
     ...
     */
    function getPriceToFlushCustomMetadata(string memory _identity) public view returns (uint256 price) {
        price = (10 * Evvm(evvmAddress.current).getRewardAmount()) * identityDetails[_identity].customMetadataMaxSlots;
    }
     ...
     */
    function getPriceToFlushUsername(string memory _identity) public view returns (uint256 price) {
        price = ((10 * Evvm(evvmAddress.current).getRewardAmount()) * identityDetails[_identity].customMetadataMaxSlots)
            + Evvm(evvmAddress.current).getRewardAmount();
    }
     ...
     */
    function checkIfNameServiceNonceIsAvailable(address _user, uint256 _nonce) public view returns (bool) {
    ...
    function isUsernameAvailable(string memory _username) public view returns (bool) {
        if (identityDetails[_username].expireDate == 0) {
            ...
            return identityDetails[_username].expireDate + 60 days < block.timestamp;
        }
    }
     ...
     */
    function getIdentityBasicMetadata(string memory _username) public view returns (address, uint256) {
        return (identityDetails[_username].owner, identityDetails[_username].expireDate);
    }
     ...
     */
    function getAmountOfCustomMetadata(string memory _username) public view returns (uint256) {
     ...
     */
    function getFullCustomMetadataOfIdentity(string memory _username) public view returns (string[] memory) {
        string[] memory _customMetadata = new string[](identityDetails[_username].customMetadataMaxSlots);
        for (uint256 i = 0; i < identityDetails[_username].customMetadataMaxSlots; i++) {
     ...
     */
    function getCustomMetadataMaxSlotsOfIdentity(string memory _username) public view returns (uint256) {
        return identityDetails[_username].customMetadataMaxSlots;
    }
     ...
     */
    function getOffersOfUsername(string memory _username) public view returns (OfferMetadata[] memory offers) {
     ...
     */
    function getLengthOfOffersUsername(string memory _username) public view returns (uint256 length) {
        do {
            length++;
     ...
     */
    function getExpireDateOfIdentity(string memory _identity) public view returns (uint256) {
    ...
    {
        return (amountToWithdrawTokens.proposal, amountToWithdrawTokens.timeToAccept);
    }
    ...
    {
        return (evvmAddress.current, evvmAddress.proposal, evvmAddress.timeToAccept);
    }
}
