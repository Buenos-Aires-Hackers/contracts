...
contract Evvm is EvvmStorage {
    ...
    fallback() external {
        ...
        assembly {
            ...
            // If it failed, revert
            default { return(0, returndatasize()) } // If it worked, return
        }
    }
     ...
     */
    function payMultiple(PayData[] memory payData)
        external
        returns (uint256 successfulTransactions, uint256 failedTransactions, bool[] memory results)
    {
        ...
        for (uint256 iteration = 0; iteration < payData.length; iteration++) {
            ...
            if (payData[iteration].priorityFlag) {
                ...
                if (!asyncUsedNonce[payData[iteration].from][payData[iteration].nonce]) {
                    asyncUsedNonce[payData[iteration].from][payData[iteration].nonce] = true;
                } else {
                    failedTransactions++;
                    results[iteration] = false;
                    continue;
                ...
                if (nextSyncUsedNonce[payData[iteration].from] == payData[iteration].nonce) {
                    nextSyncUsedNonce[payData[iteration].from]++;
                } else {
                    failedTransactions++;
                    results[iteration] = false;
            ...
            to_aux = !Strings.equal(payData[iteration].to_identity, "")
                ? NameService(nameServiceAddress).verifyStrictAndGetOwnerOfIdentity(payData[iteration].to_identity)
                : payData[iteration].to_address;
            ...
            if (!_updateBalance(payData[iteration].from, to_aux, payData[iteration].token, payData[iteration].amount)) {
                failedTransactions++;
                results[iteration] = false;
                continue;
            } else {
                if (payData[iteration].priorityFee > 0 && isAddressStaker(msg.sender)) {
                    if (
                        !_updateBalance(
                            payData[iteration].from,
                            msg.sender,
    ...
    function dispersePay(
        ...
        for (uint256 i = 0; i < toData.length; i++) {
            ...
            if (!Strings.equal(toData[i].to_identity, "")) {
                if (NameService(nameServiceAddress).strictVerifyIfIdentityExist(toData[i].to_identity)) {
                    to_aux = NameService(nameServiceAddress).getOwnerOfIdentity(toData[i].to_identity);
                }
            } else {
     ...
     */
    function disperseCaPay(DisperseCaPayMetadata[] memory toData, address token, uint256 amount) external {
        uint256 size;
        address from = msg.sender;
     ...
     */
    function _updateBalance(address from, address to, address token, uint256 value) internal returns (bool) {
    ...
    function _giveReward(address user, uint256 amount) internal returns (bool) {
        ...
        balances[user][evvmMetadata.principalTokenAddress] = userBalance + principalReward;

        return (userBalance + principalReward == balances[user][evvmMetadata.principalTokenAddress]);
    }
    ...
    function recalculateReward() public {
        if (evvmMetadata.totalSupply > evvmMetadata.eraTokens) {
            ...
            balances[msg.sender][evvmMetadata.principalTokenAddress] += evvmMetadata.reward * getRandom(1, 5083);
