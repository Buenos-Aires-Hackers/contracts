...
import {ExternalChainStationStructs} from
    "@evvm/testnet-contracts/contracts/treasuryTwoChains/lib/ExternalChainStationStructs.sol";
...
import {IInterchainGasEstimation} from
    "@axelar-network/axelar-gmp-sdk-solidity/contracts/interfaces/IInterchainGasEstimation.sol";
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";

contract TreasuryExternalChainStation is ExternalChainStationStructs, OApp, OAppOptionsType3, AxelarExecutable {
    ...
    bytes options = OptionsBuilder.addExecutorLzReceiveOption(OptionsBuilder.newOptions(), 200_000, 0);
    ...
    constructor(address _admin, CrosschainConfig memory _crosschainConfig, uint256 _evvmId)
        OApp(_crosschainConfig.endpointAddress, _admin)
        Ownable(_admin)
        AxelarExecutable(_crosschainConfig.gatewayAddress)
    {
        admin = AddressTypeProposal({current: _admin, proposal: address(0), timeToAccept: 0});
        hyperlane = HyperlaneConfig({
            hostChainStationDomainId: _crosschainConfig.hostChainStationDomainId,
        ...
        axelar = AxelarConfig({
            hostChainStationChainName: _crosschainConfig.hostChainStationChainName,
            hostChainStationAddress: "",
        ...
        onlyAdmin
    {
        ...
        hyperlane.hostChainStationAddress = bytes32(uint256(uint160(hostChainStationAddress)));
        layerZero.hostChainStationAddress = bytes32(uint256(uint160(hostChainStationAddress)));
        axelar.hostChainStationAddress = hostChainStationAddressString;
        _setPeer(layerZero.hostChainStationEid, layerZero.hostChainStationAddress);
        ...
        payable
    {
        ...
        if (protocolToExecute == 0x01) {
            ...
            IMailbox(hyperlane.mailboxAddress).dispatch{value: quote}(
                hyperlane.hostChainStationDomainId, hyperlane.hostChainStationAddress, payload
            );
            ...
            // 0x03 = Axelar
            IAxelarGasService(axelar.gasServiceAddress).payNativeGasForContractCall{value: msg.value}(
                address(this), axelar.hostChainStationChainName, axelar.hostChainStationAddress, payload, msg.sender
            );
            gateway().callContract(axelar.hostChainStationChainName, axelar.hostChainStationAddress, payload);
        } else {
            revert();
        }
    }
    ...
    function depositCoin(address toAddress, uint256 amount, bytes1 protocolToExecute) external payable {
        ...
        if (protocolToExecute == 0x01) {
            ...
            // 0x03 = Axelar
            IAxelarGasService(axelar.gasServiceAddress).payNativeGasForContractCall{value: msg.value - amount}(
                address(this), axelar.hostChainStationChainName, axelar.hostChainStationAddress, payload, msg.sender
            );
            gateway().callContract(axelar.hostChainStationChainName, axelar.hostChainStationAddress, payload);
        } else {
    ...
    function fisherBridgeSendERC20(
        ...
        emit FisherBridgeSend(
            from, addressToReceive, tokenAddress, priorityFee, amount, nextFisherExecutionNonce[from] - 1
        );
    }
    ...
    function fisherBridgeSendCoin(
        ...
        emit FisherBridgeSend(
            from, addressToReceive, address(0), priorityFee, amount, nextFisherExecutionNonce[from] - 1
        );
    }
    ...
    function getQuoteHyperlane(address toAddress, address token, uint256 amount) public view returns (uint256) {
        return IMailbox(hyperlane.mailboxAddress).quoteDispatch(
    ...
    function quoteLayerZero(address toAddress, address token, uint256 amount) public view returns (uint256) {
        MessagingFee memory fee =
            _quote(layerZero.hostChainStationEid, PayloadUtils.encodePayload(token, toAddress, amount), options, false);
    ...
    function proposeFisherExecutor(address _newFisherExecutor) external onlyAdmin {
        if (_newFisherExecutor == address(0) || _newFisherExecutor == fisherExecutor.current) revert();
    ...
    function acceptHostChainAddress() external {
        ...
        hyperlane.hostChainStationAddress =
            bytes32(uint256(uint160(hostChainAddressChangeProposal.porposeAddress_AddressType)));
        layerZero.hostChainStationAddress =
            bytes32(uint256(uint160(hostChainAddressChangeProposal.porposeAddress_AddressType)));
        axelar.hostChainStationAddress = hostChainAddressChangeProposal.porposeAddress_StringType;
    ...
    function getFisherExecutor() external view returns (AddressTypeProposal memory) {
    ...
    function getHyperlaneConfig() external view returns (HyperlaneConfig memory) {
        return hyperlane;
    }
    ...
    function getLayerZeroConfig() external view returns (LayerZeroConfig memory) {
        return layerZero;
    }
