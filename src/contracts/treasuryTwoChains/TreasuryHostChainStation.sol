...
import {HostChainStationStructs} from
    "@evvm/testnet-contracts/contracts/treasuryTwoChains/lib/HostChainStationStructs.sol";
...
contract TreasuryHostChainStation is HostChainStationStructs, OApp, OAppOptionsType3, AxelarExecutable {
    ...
    constructor(address _evvmAddress, address _admin, CrosschainConfig memory _crosschainConfig)
        ...
        AxelarExecutable(_crosschainConfig.gatewayAddress)
    {
        ...
        hyperlane = HyperlaneConfig({
            externalChainStationDomainId: _crosschainConfig.externalChainStationDomainId,
        ...
        axelar = AxelarConfig({
            externalChainStationChainName: _crosschainConfig.externalChainStationChainName,
            externalChainStationAddress: "",
    ...
    function _setExternalChainAddress(
        ...
        hyperlane.externalChainStationAddress = bytes32(uint256(uint160(externalChainStationAddress)));
        layerZero.externalChainStationAddress = bytes32(uint256(uint160(externalChainStationAddress)));
        axelar.externalChainStationAddress = externalChainStationAddressString;
        _setPeer(layerZero.externalChainStationEid, layerZero.externalChainStationAddress);
    ...
    function withdraw(address toAddress, address token, uint256 amount, bytes1 protocolToExecute) external payable {
        ...
        if (protocolToExecute == 0x01) {
            ...
            IMailbox(hyperlane.mailboxAddress).dispatch{value: quote}(
                hyperlane.externalChainStationDomainId, hyperlane.externalChainStationAddress, payload
            );
            ...
            gateway().callContract(axelar.externalChainStationChainName, axelar.externalChainStationAddress, payload);
        } else {
            revert();
        }
    }
    ...
    function quoteLayerZero(address toAddress, address token, uint256 amount) public view returns (uint256) {
        MessagingFee memory fee = _quote(
            layerZero.externalChainStationEid, PayloadUtils.encodePayload(token, toAddress, amount), options, false
        );
    ...
    function acceptExternalChainAddress() external {
        ...
        hyperlane.externalChainStationAddress =
            bytes32(uint256(uint160(externalChainAddressChangeProposal.porposeAddress_AddressType)));
        layerZero.externalChainStationAddress =
            bytes32(uint256(uint160(externalChainAddressChangeProposal.porposeAddress_AddressType)));
        axelar.externalChainStationAddress = externalChainAddressChangeProposal.porposeAddress_StringType;
        _setPeer(layerZero.externalChainStationEid, layerZero.externalChainStationAddress);
    }
    ...
    function decodeAndDeposit(bytes memory payload) internal {
        (address token, address from, uint256 amount) = PayloadUtils.decodePayload(payload);
    ...
    function executerEVVM(bool typeOfExecution, address userToExecute, address token, uint256 amount) internal {
        if (typeOfExecution) {
            // true = add
