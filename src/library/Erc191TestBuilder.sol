...
library Erc191TestBuilder {
    ...
    function buildMessageSignedForPay(
        ...
        string memory messageToSign = string.concat(
            ...
            "pay",
            ",",
            _receiverAddress == address(0) ? _receiverIdentity : AdvancedStrings.addressToString(_receiverAddress),
            ",",
    ...
    function buildMessageSignedForRenewUsername(uint256 evvmID, string memory _username, uint256 _nameServiceNonce)
        internal
        pure
        returns (bytes32 messageHash)
    {
        return buildHashForSign(
            string.concat(
                Strings.toString(evvmID), ",", "renewUsername", ",", _username, ",", Strings.toString(_nameServiceNonce)
            )
        );
    }
    ...
    function buildMessageSignedForFlushCustomMetadata(uint256 evvmID, string memory _username, uint256 _nonce)
        internal
        pure
        returns (bytes32 messageHash)
    {
        return buildHashForSign(
            string.concat(
                Strings.toString(evvmID), ",", "flushCustomMetadata", ",", _username, ",", Strings.toString(_nonce)
            )
        );
    }

    function buildMessageSignedForFlushUsername(uint256 evvmID, string memory _username, uint256 _nonce)
        internal
        pure
        returns (bytes32 messageHash)
    {
        return buildHashForSign(
            string.concat(Strings.toString(evvmID), ",", "flushUsername", ",", _username, ",", Strings.toString(_nonce))
        );
    }
    ...
    function buildHashForSign(string memory messageToSign) internal pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n", Strings.toString(bytes(messageToSign).length), messageToSign
            )
        );
    }

    function buildERC191Signature(uint8 v, bytes32 r, bytes32 s) internal pure returns (bytes memory) {
