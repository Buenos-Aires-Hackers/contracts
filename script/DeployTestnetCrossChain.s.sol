...
import {TreasuryExternalChainStation} from
    "@evvm/testnet-contracts/contracts/treasuryTwoChains/TreasuryExternalChainStation.sol";
import {TreasuryHostChainStation} from
    "@evvm/testnet-contracts/contracts/treasuryTwoChains/TreasuryHostChainStation.sol";
...
contract DeployTestnetCrossChain is Script {
    ...
    HostChainStationStructs.CrosschainConfig _crosschainConfigHost = HostChainStationStructs.CrosschainConfig({
    ...
    ExternalChainStationStructs.CrosschainConfig _crosschainConfigExternal = ExternalChainStationStructs
        .CrosschainConfig({
    ...
    function run() public {
        ...
        if (block.chainid == 11155111) {
            ...
            estimator = new Estimator(ADMIN, address(evvm), address(staking), ADMIN);
            nameService = new NameService(address(evvm), ADMIN);

            treasuryHost = new TreasuryHostChainStation(address(evvm), ADMIN, _crosschainConfigHost);

            staking._setupEstimatorAndEvvm(address(estimator), address(evvm));
            evvm._setupNameServiceAndTreasuryAddress(address(nameService), address(treasuryHost));
            ...
            treasuryExternal = new TreasuryExternalChainStation(ADMIN, _crosschainConfigExternal, 0);
